...
export const TK = {
  ...
  IOV_THREAD_NUM: 3,   // mod: was 4 -- 9 racers total (memory/thread pressure at ps1_prepare)
  UIO_THREAD_NUM: 3,   // mod: was 4
...
export function makeHarness(X) {
  ...
  // mod: shared FIFO lock for build+run chain sequences (see poops.html)
  const chainLock =
    typeof X.chainLock === "function" ? X.chainLock : (fn) => fn();
  ...
  async function signal(g, deadlineMs, why) {
    // mod: whole body under chainLock -- build (add_syscall_ret) and run
    // (runChain) must be atomic against other chain users (bridge sends)
    return await chainLock(async () => {
  ...
  async function closeWakeGates(g) {
    // mod: whole body under chainLock -- build+run atomic vs bridge sends
    return await chainLock(async () => {
    const fds = [];
...
export const PK = {
  ...
  KSLOW_ATTEMPTS: 5,   // mod: was 3 -- more tries to catch a good race alignment on hard boots
...
export function makePoopsEngine(X) {
  ...
  async function runBuilt(where, build) {
    // mod: whole body under chainLock -- the build() closure mutates the
    // shared chain buffer and must not interleave with another caller's
    // frames (bridge sends after elfldr is up)
    return await chainLock(async () => {
      assertFresh(where);
      try {
        build();
  ...
  // mod: conservative post-kernel release switch (WORK_REPORT 7.5 step 1).
  // true  = after kernel writes, terminate the three racer groups and let
  //         terminate() close their wake-gate pipes (about -18 fds), then
  //         park everything else exactly as before;
  // false = old behavior (park everything, no termination attempt).
  // Rollback = flip to false. The socket bank, race socketpairs,
  // master/victim pipes and kqueues are NEVER touched either way.
  const CONS_RELEASE_RACERS = true;
  ...
  async function cleanup(opts) {
    ...
    if (S.kernelWrites !== 0 || S.jailbroken) {
      // mod: conservative release before parking. terminate() retargets
      // each racer's pivot slot to its exit tail (pure userspace stack
      // write), wakes it through its own wake-gate pipe, waits for
      // ST_EXITED and closes the gate fds. No unblockGroup() here on
      // purpose: it writes/reads the race socketpairs (S.iovSock*/S.uioSock*)
      // and those must not be poked after kernel writes. A group that
      // does not confirm exit falls back to the old park behavior.
      const consReleased = [];
      const consFailed = [];
      const consLive = [];
      if (CONS_RELEASE_RACERS) {
        for (const g of [S.iovGroup, S.uioReadGroup, S.uioWriteGroup]) {
          if (!g || !g.spawned || !g.spawned.length) continue;
          try {
            if (g.terminated) {
              consReleased.push(g.name + "(already)");
              continue;
            }
            forceSettled(g, "cons-release");
            const r = await H.terminate(g, "cons-release");
            let notExited = 0;
            for (const t of g.spawned)
              if (g.S.get("status", t.jbWid) !== TK.ST_EXITED) notExited++;
            if (
              !r.already &&
              ((r.exitedMs !== undefined && r.exitedMs < 0) || notExited > 0)
            )
              throw new Error(
                "exitedMs=" + r.exitedMs + "-notExited=" + notExited +
                  "/" + g.spawned.length,
              );
            consReleased.push(
              g.name + (r.already ? "(already)" :
                "(exited in " + r.exitedMs + " ms, gate fds closed " +
                  (r.gates ? r.gates.closed : -1) +
                  (r.gates && r.gates.failed.length
                    ? ", close failed: " + r.gates.failed.join(",")
                    : "") + ")"),
            );
          } catch (err) {
            consLive.push(g.name);
            consFailed.push(
              g.name + ": " +
        ...
        for (const g of [S.iovGroup, S.uioReadGroup, S.uioWriteGroup])
          if (g && g.spawned && g.spawned.length) consLive.push(g.name);
      }

      const parkedGroups = consLive.slice();
      ...
      for (const s of consReleased) rep.groups.push("cons-release: " + s);
      for (const s of consFailed) rep.groups.push("cons-release FAILED: " + s);
      ...
      rep.notes.push(
        "mod: conservative release " +
          (CONS_RELEASE_RACERS
            ? "ran for racers + wake gates (" +
              consReleased.length + " released, " +
              consLive.length + " still live)"
            : "disabled") +
          "; socket bank/pipes/pairs stay parked after kernel writes; " +
          "reboot before running again",
      );
      state.cleanupFailed =
        (state.cleanupFailed ? state.cleanupFailed + "  " : "") +
        "post-kernel state parked (conservative release: " +
          consReleased.length + " released, " +
          consLive.length + " live); reboot required";
      rep.doNotClose = Array.from(S.doNotClose);
      flushMark(
        "POOPS-CLEANUP-PARKED",
        "kernelWrites=" + S.kernelWrites +
          "-jailbroken=" + (S.jailbroken ? 1 : 0) +
          "-consReleased=" + consReleased.length +
          "-consLive=" + (parkedGroups.join(".") || "none") +
  ...
  L.push({
    ...
    async run() {
      const wanted = [
        ...
        [PSYS.THR_NEW, (TK.IOV_THREAD_NUM + TK.UIO_THREAD_NUM * 2) + " racers"],
        [PSYS.THR_EXIT, "kill switch tail"],
        [PSYS.READ, "racers park in read() on their wake gate"],
        [
          PSYS.PIPE2,
          "master/victim pipes and " + (TK.IOV_THREAD_NUM + TK.UIO_THREAD_NUM * 2) + " racer wake " +
